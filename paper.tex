...
The Table \ref{meanFitness} shows the mean fitness values accomplished by two approaches and Table \ref{meanTime} shows the mean execution time required by them. We employ statistical analysis to test the significant differences in mean fitness value and execution time.  The results show that the PSO-based approach performs better in four of five tasks. 
...
We study the mean fitness value with the optimum for both PSO-based and GP-based approach over 30 independent runs. Fig. \ref{psovsgp} is an example of convergence rate performance in WSC09 03, which shows the behaviour of PSO-based approach manifests the characteristics of fast convergence and reaches a better-optimised solution, while the performance of GP is barely satisfactory.

\begin{figure}[h]
\centering
\fbox{\includegraphics[scale=.29]{psovsgp.pdf}}
