...
\begin{algorithm}
 ...
 Randomly initialise each particle in the swarm\;
  \While {max. iterations not met}{
     ...
     Decode the service into a weighted DAG \;
...
\begin{algorithm}
 ...
 $OutputSet \leftarrow$ \{$I_{T}$\}\;
  \While { $O_{T}$ $\notin$ $OutputSet$  and $queue !=null$}{
     \ForEach{$S$ in $queue$}{
      \uIf{$fullmatch(a \Rightarrow b)$$\wedge$ $(a \in OutputSet)$ $\wedge$ $(b \in I_{S})$ $\wedge$ $\mid fullmatch(a \Rightarrow b) \mid = \mid I_{S} \mid$}{    
    		 \ForEach{$fullmatch(a \Rightarrow b)$}{
    		$sim(a,b) \leftarrow$ query $IndexCache$\;
 			$sm(a,b) \leftarrow$ $( type(a,b), sim(a,b))$\;
              }
        $sm_{e} \leftarrow$ aggregate \{ $sm(a,b)$ \}\;
        $E \leftarrow$ add $e$\;
        $V \leftarrow$ add $S$\;
        $OutputSet$ add \{$O_{S}$\}\;    
        $queue$.remove $S$\;
