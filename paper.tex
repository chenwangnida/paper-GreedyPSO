...
Although obtaining a correct combination of web service is essential, there are many ways to chain web services to reach users' needs with different web services in a service repository. These web services have different QoS (such as availability, price, reliability and response time ). Moreover, the valid semantic matches between two web services could be associated with different semantic matchmaking quality. For example, Generate Map service produce StreetMap which is considered to be a valid semantic match to TouristMap required by users. Fig .\ref{taxonomy} represents a taxonomy of all the inputs and  the outputs described in Fig. \ref{motivation} for a travel planning domain, where the concept Map and its sub-concept urbanMap are assigned with a touristMap instance and streetMap instance respectively. The valid semantic match is considered that an instance of urban Map must be an instance of Map, not vice versa. If a web service $GenerateTouristMap$ found in the service repository that produces TouristMap as an output. I consider its semantic matchmaking quality is higher than that of $GenerateMap$. However, $GenerateTouristMap$ might have a very bad QoS.
...
We herein give formal definitions of the assumptions the concepts regarding the semantic web service composition problem utilised in our approach that consists of three main concepts: a semantic web service model, a semantic matchmaking model, a weighted DAG composition model, and QoS calculation model.
\vspace{0.3cm}

\textbf{Semantic Web Service Model}. The semantic web service model represent web services with functional descriptions, and how interoperability of web services enable matchmaking to automatically generate composition solutions.
...
Four most often considered QoS parameters \cite{zeng2003quality} are response time, cost, reliability and availability described as follows:\textit{Response time} ($T$) measures the expected delay in seconds between the moment when a request is sent and the moment when the results are received.\textit{Cost} ($C$) is the amount of money that a service requester has to pay for executing the web service. \textit{Reliability} ($R$) is the probability that a request is correctly responded within the maximum expected time frame. \textit{Availability} ($A$) is the probability that a web service is accessible.

	The aggregation value of QoS in a DAG with respect to different constructs, which reflects how services associated with each other in a service composition. Here we consider two composition constructs: sequence and parallel constructs, in building the composition web service. The QoS calculation models are described as follows:


\textbf{Definition 5.} $V_{A}(WG)$ is a function that given a $WG$, a aggregated $availability$ value is returned, which is defined as :
$V_{A}(WG)=\prod\limits^n_{k=1}a_k$, for all ${S_1, S_2, ... S_n} \subset V$.

\textbf{Definition 6.} $V_{R}(WG)$ is a function that given a $WG$, a aggregated $reliability$ value is returned, which is defined as :
$V_{R}(WG)=\prod\limits^n_{k=1}r_k$, for all ${S_1, S_2, ... S_n} \subset V$.

\textbf{Definition 7.} $V_{C}(WG)$ is a function that given a $WG$, a aggregated cost value is returned, which is defined as :
$V_{C}(WG)=\sum\limits^n_{k=1}c_k$, for all ${S_1, S_2, ... S_n} \subset V$.


\textbf{Definition 8.} $V_{T}(WG)$ is a function that given a $WG$, a aggregated time value is returned, where  $T$ is determined by the most time-consuming path in the composition flow of the solution, which is defined as :
$V_{T}(WG)$ $=MAX\{timePath(WG)_n$ $|n\in\{1,\ldots,m\}\}$, where m is the number of $timePath$.

\vspace{0.5cm}

\textbf{Composition Problem}
A set of semantic web services $S$ are chained as a weighted DAG composition mode $WG$ when a composition task $Comp.T$ is requested by users. The composition problem considered in this paper satisfying the follows:
