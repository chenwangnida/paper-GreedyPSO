 ...
 \LinesNumbered
	...
	\nonl \While {max. iterations not met}{
		\ForAll{particles in the swarm}{
			...
			{\textbf{4.} $WG \leftarrow$ generatedWeigthedGraph().}\\
			{\textbf{5.} Calculate the $WG$ fitness value.}\\
...
\begin{algorithm}
 ...
 \Input{Task inputs $I$, task outputs $O$, relevant Web services $queue$, IndexCache $IndexCache$}
  ...
  \While {all $O$ $\notin$ $OutputSet$  and $queue !=null$}{
     \ForEach{$ws$ in $queue$}{
      \uIf{$ws.inputs$ $\in$ $OutputSet$ }{    
    		...
    		$parameter.matchType \leftarrow$ query $IndexCache$\;
     		$parameter.conceptSiminarity \leftarrow$ query $IndexCache$\;
              }
        $WG.edge \leftarrow$ new $Edge$()\;
        $edge.weights \leftarrow$ aggregation($parameters$)\;
