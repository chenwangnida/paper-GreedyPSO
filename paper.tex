...
The fitness value of a service composition solution is a weighted sum of all the quality components discussed previously shown in Formula \ref{equation6}, in which fitness value of 1 means the best comprehensive quality and 0 means the worst. As the sum of all the components' weights ($w_1$ to $w_6$) equals to 1, $MT$, $S$, $A$, $R$, $T$, and $C$ must be normalised so that the fitness value falls within the range from 0 to 1. Also, the lowest T and C value represent the best component quality so that the objective function can be offset using $(1 - T)$ and $(1 - C)$.
...
\begin{algorithm}[!htb]
 ...
 Create $start$ vertex with outputs $I$ and $end$ vertex with inputs $O$\;
 Create graph $WG$ containing the $start$ vertex\;
 Create available outputs set containing $start$ outputs\;
 ...
 \textcolor{blue}{Create edge assigned with weight\;}
 \textcolor{blue}{Connect vertex to graph with the edge\;}
 Remove it from queue and go back to the queue's beginning\;
 }}
 Connect end vetex\;
 Remove dangling vertices from graph $WG$\;
 Remove dangling edges from graph $WG$
 \KwRet $WG$\;
 \caption{Create a composition weighted graph from a queue}
\label{graph_building}
\end{algorithm}

The overall algorithm investigated here is a PSO-based web service composition algorithm \ref{novelSteps} from   \cite{da2016particle} with a different decoded algorithm \ref{graph_building} to generate a composition weighted graph from a queue. In the first algorithm, the idea is to translate the particle location into a service queue as an indirect representation of composition weighted graph, so finding the best fitness of the composite service solution is to discover the optimised location of the particle. In PSO, the dimension of the particle is set up as the same as the number of relevant web services, and all the services index is mapped to the location in a particle, and we put services in a queue in ascending order, from which we decode a weighted graph using Algorithm \ref{graph_building}. We select and connect web services to the start Vetex from the service queue if the web service can satisfy the given $T_{input}$. Meanwhile, an Output Set is initialised and kept being updated using outputs from the new-added web services in the graph, later on, more services are connected if services' inputs could be satisfied by any output in the Output Set. At last, The end vetex is connected if the updated Output Set contains the $T_{output}$. In addition, dangle service nodes and edges should be removed. 
