  ...
  \While { $O_{T}$ do not satisfied by $OutputSet$}{
     \ForEach{$S$ in $queue$}{
      \uIf{all $I_{S}$ satisfied by $OutputSet$}{    
        $e \leftarrow$ calculate $type_e$, $sim_e$\;
        $E$ add $e$\;
        $V$ add $S$\;
        $OutputSet$ add \{$O_{S}$\}\;    
        $queue$.remove $S$\;
...
$(2)$ measure the effectiveness of our proposed comprehensive quality to achieve a desirable balance on semantic matchmaking quality and QoS, see comparison test in \ref{comparisonTest}.
