...
\textbf{Definition 3.} Given $a, b \in C$, $type(a, b)$ returns the matchmaking types of between two concepts related to outputs and inputs of two difference web services, and this matchmaking type is determined by the logic described in the $O$. 

If both $a$ and $b$ are equivalent, $exact$ type ($a \equiv b$) is returned.
If the concept of $a$ is a sub-concept of the concept of $b$ type ($a \sqsubseteq b$) is returned. 
If the concept of $a$ is a super-concept of the concept of $b$, $subsumes$ type ($a \sqsupseteq b$) is returned. 
If none of previous matchmaking types are returned, $fail$ type ($a \perp b$) is returned.

\textbf{Definition 4.} Given $(a \in O^{C}_{S_m}) \vee (b \in I^{C}_{S_n}) \wedge (m \neq n) $, a $full match(a \rightarrow b)$ holds if $type(a, b) \neq subsumes \mid fail$. The operator ``$ \rightarrow$'' means $a$ is passed to $b$. The full match is a guarantee of completely valid  ``$ \rightarrow$'' operation as $b$ is fully satisfied by $a$.
...
$E = \{e_{1}, e_{2},... e_{n} \}$ is a set of edges. where $e = O_{S_{m}} \cup I_{S_{n}} \cup $ $fullmatch(O_{m} \rightarrow I_{n})$ is source service $S_m$ and target service $s_n$
