...
\begin{algorithm}
 ...
 Randomly initialise each particle in the swarm\;
  \While {max. iterations not met}{
     ...
     Map service candidates to particle's position vector\;
     Create a service queue and sort it by position vectors' weights\;
     Create the service into a weighted DAG \;
...
This Algorithm  \ref{graph_building} is a forward graph graph buidling algorithm based on \cite{blum1997fast}. This algorithm take one input as the sorted service queues from step 5 of Algorithm \ref{novelSteps}. If service queues are sorted with different service order, it is possible to create different corresponding weighted DAGs as composition solutions. In addition. $I_{T}$, $O_{T}$ and $Cache$ are also taken as inputs. Firstly, $Start$ and $End$ are added to V of WG as an initialisation, and a $OutputSet$ is also created with $I_{T}$.  If all the inputs $I_{S}$ of first popped  $S$ from $queue$ can be satisfied by provided outputs of $OutputSet$, this $S$ is added to $V$ and its outputs are added to $OutputSet$. Meanwhile, $e$ is created with its $sm_{e}$ calculated based on the aggregation value of all semantic matchmaking quality from each satisfied input of $S$. These steps are not stopped until all  $O_{T}$ can satisfied from $Outputset$ or queue is null. Consequently, this forward graph building technique could lead to more services and edges connected to the $WG$, which should be removed before $WG$ is returned.
...
Web service challenge 2009 (WSC09) \cite{kona2009wsc} is a benchmark dataset consisting of five composition tasks that associated with an increasing number of services, and concepts in ontologies. An augmented version of WSC09 is utilised including QoS attributes for services, this version of WSC09 dataset is used in \cite{ma2015hybrid,da2016genetic}

The parameters were chosen based on general settings from \cite{shi2001particle} for our PSO-based approach, In particular, PSO population size is 30 with 100 generations. We run 30 times independently for each dataset. We configure weight of fitness function to properly balance functional side and nonfunctional side. Therefore, $w_{1}$ and $w_{2}$ are set equally to 0.25, and $w_{3}$, $w_{4}$, $w_{5}$, $w_{6}$ are all set to 0.125 accordingly. The $type(a,b)$ is set to 1 ($Exact$) and 0.75 ($Plugin$) according to \cite{lecue2009optimizing}. In general, weight settings and parameter match type quality are decided by users' preferences.
...
The first column of the table \ref{meanFitness} shows five tasks from WSC09 Dataset. The second and third column of the table \ref{meanFitness} show the orginal service repository size and shrunk service repository size after the greedy search respectively regarding the five tasks for the initialisation discussed in step 2 SubSect.\ref{PSO_based_approach}. These significant decreased number of services demonstrate that the researching space is effectively reduced. The fourth and fifth column of the table \ref{meanFitness} show the mean fitness values of 30 independent runs accomplished by two methods. We employ statistical analysis to test the significant differences in mean fitness value. The results show that the PSO-based approach performs better in four of five tasks (all the p-values are consistently smaller than 0.01).
...
\hline
\multicolumn{1}{c|}{WSC09} &Original $SR$  &Shrunk $SR$   &PSO-based approach & GP-based approach  \\ \hline
